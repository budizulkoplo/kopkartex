                                            ...
                                            <table class="table table-sm table-striped table-bordered mb-2 align-middle" id="tabelBarang" style="font-size: small;">
                                                ...
                                                <tbody>
                                                    ...
                                                    <tr data-detail-id="{{ $detail->id }}" data-id="{{ $detail->barang_id }}" data-jenis="barang">
                                                        ...
                                                        <td class="cell-select-barang">
                                                            <select class="form-select form-select-sm namabarang" style="width:100%" required>
                                                                <option value="{{ $detail->barang_id }}"
                                                                        selected
                                                                        data-code="{{ $detail->barang->kode_barang ?? '' }}"
                                                                        data-stok="{{ $detail->barang->stok?->stok ?? 0 }}"
                                                                        data-harga-jual="{{ $detail->harga }}"
                                                        ...
                                                        <td>
                                                            <input type="number" name="diskon_item[]" class="form-control form-control-sm diskonitem" value="{{ $detail->diskon ?? 0 }}" min="0" step="0.01" onfocus="this.select()" onkeyup="kalkulasi()" onchange="kalkulasi()">
                                                        </td>
                                        ...
                                        <div class="table-responsive bengkel-table-wrap">
                                            <table class="table table-sm table-striped table-bordered mb-2 align-middle" id="tabelJasa" style="font-size: small;">
                                                ...
                                                <tbody>
                                                    ...
                                                    @php
                                                        $jasaQtyValue = rtrim(rtrim(number_format((float) $detail->qty, 3, '.', ''), '0'), '.');
                                                    @endphp
                                                    <tr data-detail-id="{{ $detail->id }}" data-jenis="jasa">
                                                        ...
                                                        <td>
                                                            <input type="number" name="jasa_qty[]" class="form-control form-control-sm jasaqty" value="{{ $jasaQtyValue }}" min="0.001" step="0.001" onfocus="this.select()" onkeyup="kalkulasi()">
                                                        </td>
                                                        <td>
                        ...
                        <div class="row mt-3">
                            <div class="col-md-4">
                                ...
                                <div class="input-group input-group-sm mb-2">
                                    ...
                                    <span class="input-group-text">Rp.</span>
                                    <input type="number" class="form-control" value="{{ $transaksi->diskon }}" name="diskon" id="diskon" min="0" step="0.01" onkeyup="kalkulasi()" onchange="kalkulasi()">
                                </div>
                                <div class="input-group input-group-sm mb-2">
        ...
        <script>
            ...
            function addBarangRow(datarow = null, qty = 1) {
                ...
                let newRow = $(`
                    <tr data-id="${datarow ? datarow.id : 0}">
                        ...
                        <td>
                            <select class="form-select form-select-sm namabarang" style="width:100%" required>
                                ${datarow ? `<option value="${datarow.id}" selected data-code="${datarow.code || ''}" data-stok="${datarow.stok || 0}" data-harga-jual="${datarow.harga_jual || 0}" data-cicilan="${datarow.kategori_cicilan || 1}">${datarow.text}</option>` : ''}
                            </select>
            ...
            function initSelect2Barang(context) {
                function normalizeBarangData(data) {
                    ...
                    data.code = data.code ?? option.dataset.code ?? '';
                    data.stok = data.stok ?? parseFloat(option.dataset.stok ?? 0);
                    data.harga_jual = data.harga_jual ?? parseFloat(option.dataset.hargaJual ?? 0);
                    data.kategori_cicilan = data.kategori_cicilan ?? parseFloat(option.dataset.cicilan ?? 1);

                    return data;
                }

                function buildBarangLabel(data) {
                ...
                context.find('.namabarang').select2({
                    ...
                    selectedOption
                        ...
                        .attr('data-stok', data.stok)
                        .attr('data-harga-jual', data.harga_jual)
                        .attr('data-cicilan', data.kategori_cicilan)
                        .text(buildBarangLabel({
                            id: data.id,
                            text: data.text,
                            code: data.code,
                            stok: data.stok,
                            harga_jual: data.harga_jual,
                            kategori_cicilan: data.kategori_cicilan
                        }));
                ...
                context.find('.namabarang').each(function() {
                    const selectedOption = $(this).find('option:selected');
                    ...
                    selectedOption.text(buildBarangLabel({
                        id: selectedOption.val(),
                        text: namaBarang,
                        code: kode,
                        stok: stok,
                        harga_jual: hargaJual,
                        kategori_cicilan: selectedOption.data('cicilan') || 1,
                        element: selectedOption.get(0)
                    }));
                ...
                function processRevise() {
                    ...
                    $('#tabelBarang tbody tr').each(function() {
                        items.push({
                            ...
                            diskon: parseFloat($(this).find('.diskonitem').val()) || 0
                        });
                    });
